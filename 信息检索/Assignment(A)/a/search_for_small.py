...
# evaluation , stopwords, BM25 scores, port need to be pre-processed. they are useful to run the def querying
def evaluation(stopwords, BM25, stemmer):
    ...
    with open('output.txt', 'w') as output:   # write output file
        with open('./files/queries.txt', 'r') as f: # go through all queries
            ...
            for query in sample_queries:
                ...
                resultSet[words[0]] = querying(queries[words[0]], stopwords, BM25, "evaluation", stemmer)  # key is the
                # document id, value is BM25 score
                rankList = sorted(resultSet[words[0]], key=resultSet[words[0]].get, reverse=True)  # sort the key of
                                                                                                   # result set
                rank = 0
                for doc in rankList[:15]:     # write the first 15 retrieval document to the output file
                ...
                Rrelret = 0  # store number R  document in retrival document, R is the number of relevant document
                for doc in rankList[:15]:         # Iterate through the first fifteen result sets
                    if doc in relavance_of_queries[words[0]]:   # go through the relavant document in retrival document
                        ...
                        number_of_nonrel = number + 1 - number_of_relret     # get the number of non-relavant document
                        if number_of_nonrel > number_of_relavant[words[0]]: # 1-number_of_nonrel / number_of_relavant
                                                                            # can not less than 0
                    ...
                    number = number + 1
                Sum_precisions = Sum_precisions + number_of_relret / 15  # get the precision of one query and add them
        ...
        if sys.argv[2] == 'manual': #  create sys path of querying
            ...
            while query != "quit":   # loop until the user enter quit
                ...
                if query == "":  # the case of entering nothing
                    ...
                    print('Results for query [' + query + ']')   # print the  first fifteen result set
